...
def train_model(model, epochs, X_train, y_train):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            loss_metrics.append(float(loss))
            print(f"Number of epoch {epoch}, loss: {float(loss)}")
...
if __name__ == "__main__":
    ...
    scripted_model, loss_metrics = train_model(scripted_model, args.epochs, X_train, y_train)
    test_model(scripted_model, X_test, y_test)

    with mlflow.start_run() as run:
        print(">>> Number of Loss metrics", len(loss_metrics))
        # Log Loss metric
